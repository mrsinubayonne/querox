...
const ProfileLogin: React.FC = () => {
  ...
  // Masque les profils auto-générés (libellés « Propriétaire » / mojibake « Propri??taire »)
  // qui n'ont pas été créés manuellement par l'utilisateur.
  const isAutoGenerated = (name: string) => {
    const n = (name || '').trim().toLowerCase().replace(/[^a-z]/g, '');
    return n === 'proprietaire' || n === 'propritaire' || n === 'owner' || n === '';
  };

  const grouped = useMemo(() => {
    const map = new Map<string, { outletName: string; items: ProfileRow[] }>();
    (profiles || []).filter((p) => !isAutoGenerated(p.profile_name)).forEach((p) => {
