            ...
            <div className="flex-1 overflow-auto p-4 bg-white">
              {!currentPeriod && !selectedPeriodId ? (
                <div className="text-center py-12 text-muted-foreground">
                  <p>Aucune période active ou sélectionnée — démarrez une période pour voir l'aperçu.</p>
                </div>
              ) : (
                <InlinePreviewContent
                  ...
                  periodStart={
                    selectedPeriodId
                      ? periods.find((p) => p.id === selectedPeriodId)?.started_at
                      : currentPeriod?.started_at
                  }
                  periodEnd={
                    selectedPeriodId
                      ? periods.find((p) => p.id === selectedPeriodId)?.ended_at
                      : currentPeriod?.ended_at
                  }
                  outletId={
                    (selectedPeriodId
                      ? periods.find((p) => p.id === selectedPeriodId)?.outlet_id
                      : currentPeriod?.outlet_id) || selectedOutletId
                  }
                  outletName={
                    outlets?.find(
                      (o) =>
