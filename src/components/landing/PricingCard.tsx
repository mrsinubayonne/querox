...
const PricingCard: React.FC<PricingCardProps> = ({ plan }) => {
  ...
  const handleSubscribe = async () => {
    if (plan.isWhatsApp && plan.whatsappNumber) {
      ...
      window.open(`https://wa.me/${plan.whatsappNumber.replace(/[^0-9]/g, '')}?text=${message}`, '_blank');
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke('create-maketou-checkout', {
        body: { tier: plan.tier, period: billingPeriod },
      });
  ...
  return (
    <Card className={`relative ${plan.popular ? 'border-2 border-primary shadow-lg scale-105' : ''}`}>
      {plan.popular && (
          ...
          <Tabs value={billingPeriod} onValueChange={(value) => setBillingPeriod(value as 'monthly' | 'annual')} className="w-full">
            ...
            <TabsContent value="annual" className="mt-0">
              <div className="mt-4">
                ...
                <span className="text-muted-foreground ml-2">{plan.annualPeriod}</span>
              </div>
            </TabsContent>
          </Tabs>
        ) : (
