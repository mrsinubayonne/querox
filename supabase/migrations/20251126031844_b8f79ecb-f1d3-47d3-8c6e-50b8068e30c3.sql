-- Ensure invoices are always generated when a table session is closed or marked as paid
-- This updates the existing create_invoice_for_closed_session function to also handle
-- sessions switching to status = 'paid' even when there is no debtor linked.
...
BEGIN
  ...
  -- 2. OR status changes to 'paid' (including legacy sessions without invoices)
  IF (TG_OP = 'UPDATE' AND 
      ...
      FOR attempt IN 1..5 LOOP
        BEGIN
          ...
          INSERT INTO public.invoices (
            ...
            NEW.total_amount,
            'paid',  -- Mark as paid directly since session is closed/paid
