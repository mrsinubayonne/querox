  ...
  SELECT EXISTS (
    ...
    WHERE tm.owner_id = _owner_id
      ...
      AND (
        ...
        OR ci.email = lower('pin-' || tm.id::text || '@team.querox.local')
      )
      AND (
        EXISTS (
          SELECT 1
...
REVOKE ALL ON FUNCTION public.team_member_has_outlet_access(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.team_member_has_outlet_access(uuid, uuid) TO authenticated;

REVOKE ALL ON FUNCTION public.team_member_has_any_permission(uuid, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.team_member_has_any_permission(uuid, text[]) TO authenticated;

REVOKE ALL ON FUNCTION public.team_member_can_access(uuid, uuid, text[]) FROM PUBLIC, anon;
